...
def segment_customers(df):
    ...
    if len(features) >= 3:
        ...
        # Plotando a tabela dos segmentos
        fig, ax = plt.subplots(figsize=(12, 6))  # Ajuste o tamanho conforme necessário
        ax.axis('tight')
        ax.axis('off')
        table_data = segmentos.reset_index()
        table = Table(ax, bbox=[0, 0, 1, 1])
        n_rows, n_cols = table_data.shape
        width, height = 1.0 / n_cols, 1.0 / n_rows

        for (i, j), val in np.ndenumerate(table_data.values):
            table.add_cell(i, j, width, height, text=val, loc='center', edgecolor='w', facecolor='none')

        for j, col in enumerate(table_data.columns):
            table.add_cell(-1, j, width, height, text=col, loc='center', edgecolor='w', facecolor='lightgrey')

        ax.add_table(table)
        plt.title('Análise dos Segmentos')
        plt.show()
...
def train_logistic_regression(df):
    ...
    if df['sucesso_campanha'].nunique() > 1:  # Verifica se há pelo menos 2 classes
        ...
        print("Não há variação suficiente na variável alvo para treinar um modelo.")

def plot_results(df):
    plt.figure(figsize=(10, 6))
    df.groupby('mes_venda')['Venda'].sum().plot(kind='bar')
